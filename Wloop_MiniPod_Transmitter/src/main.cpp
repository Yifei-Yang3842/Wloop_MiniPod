...
void loop() {
  ...
  data.jStick_yaw = controllerPad.getYawVal();
  data.BLDCPower = controllerPad.getPower();

  data.mode = controllerPad.getMode1() ? 1: 2;
  data.EStop = controllerPad.getEStop();
  data.FunctionBtn = controllerPad.getFunctionBtn();
